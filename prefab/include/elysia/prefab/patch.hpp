...
struct Patch { std::string target; Object set; Object attach; };
...
inline bool apply_patch(World& world, const ComponentRegistry& registry, const Patch& patch,
    ...
    world.query<const NameTag, Entity>().each([&](const NameTag& tag, Entity e) {
        ...
        if (target) throw Error("Ambiguous patch target '" + name + "'");
        target = e;
    });
    if (!target) return false;
    std::map<uint64_t, std::pair<const archive::ComponentFactory*, Value>> updates;
    for (const auto& [key, patch_value] : patch.set) {
        ...
        if (!ptr) throw Error("Patch target lacks component '" + fac.key + "'");
        if (!fac.generic) throw Error("Component has no generic codec: " + fac.key);
    ...
    World validation;
    auto test = validation.spawn().entity;
    for (const auto& [id, update] : updates) registry.decode(validation, test, *update.first, update.second);
    for (const auto& [id, update] : updates) registry.decode(world, *target, *update.first, update.second);
    return true;
}
}
